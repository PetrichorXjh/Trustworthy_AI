...
parser.add_argument('--ep', default=50, type=int,
                    help='the epoch for model training')
parser.add_argument('--seed', default=22151330, type=int,
...
if args.optim == 'sgd':
    ...
    optimizer = torch.optim.Adam(net.parameters(), lr=lr, betas=(0.5, 0.999), weight_decay=5e-4)
else:
...
def train(epoch):
    ...
    mean_loss = 1.*train_loss / len(dataloader)
    mean_acc = 1.*correct / total
...
def test(epoch):
    ...
    mean_loss = 1.*test_loss / len(dataloader)
    mean_acc = 1.*correct / total
    return mean_loss, mean_acc
